...
require_once 'Correios/Http/CookieManager.php';
 ...
 */
class Correios_Http_HTTPCookieManager implements Correios_Http_CookieManager {
	/**
	 * @var	string
	 */
	private $cookieFile;
	...
	public function __construct( $dirname = null ) {
		...
		if ( is_readable( $dirname ) && is_writable( $dirname ) ) {
			...
			if ( !is_file( $cookieFile ) ) {
				...
				if ( $cookieManager instanceof Correios_Http_HTTPCookieManager ) {
	 ...
	 */
	public function addCookie( Correios_Http_Cookie $cookie ) {
	...
	public function setCookie( $setCookie , $domain = null ) {
		if ( is_array( $setCookie ) ) {
			...
			if ( preg_match( '/(?<name>[^\=]+)\=(?<value>[^;]+)(; expires=(?<expires>[^;]+))?(; path=(?<path>[^;]+))?(; domain=(?<domain>[^;]+))?(; (?<secure>secure))?(; (?<httponly>httponly))?/' , $setCookie , $matches ) ){
				...
				$this->cookies[ $cookieDomain ][] = new Correios_Http_Cookie( $cookieName , $cookieValue , $cookieDomain , $cookieExpires , $cookiePath , $cookieSecure );
			}
		}
	}
		...
		if ( is_array( $cookies ) ) {
			...
			foreach ( $cookies as $domain => $domainCookies ) {
				foreach ( $domainCookies as $cookie ) {
					if ( $cookie instanceof Correios_Http_Cookie ) {
